        ...
        <div className='w-fit min-w-full'>
          <table {...getTableProps()} className='w-full border dark:border-gray-600'>
            ...
            <tbody {...getTableBodyProps()}>
              ...
              {rows.slice(4, 5).map((row, index) => {
                ...
                return (
                  <tr
                    {...row.getRowProps()}
                    key={`table-body-row-${index}`}
                    className='my-4 divide-x border-b border-t bg-gray-100 last-of-type:border-b-0 rtl:divide-x-reverse dark:divide-gray-600 dark:border-gray-600 dark:bg-paper-dark-contrast'
                  >
                    {row.cells.map((cell, i) => (
                      <td {...cell.getCellProps()} key={`table-cell-${i}`}>
                        {cell.render('Cell')}
              ...
              {rows.slice(5, 8).map((row, index) => {
                prepareRow(row)
                return (
                  <tr
                    {...row.getRowProps()}
                    key={`table-body-row-${index}`}
                    className='divide-x border-b border-t last-of-type:border-b-0 rtl:divide-x-reverse dark:divide-gray-600 dark:border-gray-600'
                  >
                    {row.cells.map((cell, i) => (
                      <td {...cell.getCellProps()} key={`table-cell-${i}`}>
                        {cell.render('Cell')}

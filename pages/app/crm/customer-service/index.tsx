  ...
  return (
    <>
      ...
      <div className='flex w-full flex-col overflow-hidden px-5'>
        ...
        />
        <TabsPrimitive.Root
          defaultValue='unassined'
          dir={locale === 'ar' ? 'rtl' : 'ltr'}
          className='overflow-hidden'
        >
          <TabsPrimitive.List className='flex w-full items-center gap-4 rounded-lg border bg-gray-50 p-1 dark:bg-gray-800'>
            {TABS.map((item, i) => (
              <TabsPrimitive.Trigger
                key={i}
                value={item.value}
                className={clsx(
                  'relative flex cursor-pointer items-center justify-start gap-3 rounded-lg p-2 transition-all',
          ...
          <TabsPrimitive.Content value='unassined' className='w-full py-6'></TabsPrimitive.Content>
        </TabsPrimitive.Root>
      </div>
    </>
  )
